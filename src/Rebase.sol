 ...
 * `_getTotalVaultAssets` function. `StreamVaults.sol` contract inherits Rebase and defines
 * the `_getTotalVaultAssets` function.
 *
 * Rebased balances are dynamic and represent the holder's share in the total amount
 * of Assets controlled by the vault. Account shares aren't normalized, so the
 ...
 *
 * Since balances of all token holders change when the amount of total pooled Asset
...
abstract contract RebaseToken is IERC20 {
     ...
     * and corresponding StreamUSDC amount.
     * The StreamUSDC amount is calculated twice: before and after the burning incurred rebase.
     *
     * @param account holder of the burnt shares
     * @param preRebaseTokenAmount amount of StreamUSDC the burnt shares corresponded to before the burn
     * @param postRebaseTokenAmount amount of StreamUSDC the burnt shares corresponded to after the burn
    ...
    /**
     * @return the entire amount of Assets controlled by the protocol.
     *
     ...
     * - the caller must have allowance for `_sender`'s tokens of at least `getPooledAssetsByShares(_sharesAmount)`.
     * - the contract must not be paused.
     *
     * @dev The `_sharesAmount` argument is the amount of shares, not tokens.
     */
